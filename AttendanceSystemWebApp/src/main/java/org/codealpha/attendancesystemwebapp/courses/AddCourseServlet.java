...
public class AddCourseServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            // Creating a new Cources object
            Cources course = new Cources(courseName, instructor, startDate, endDate, capacity);
            course.setLocation(location);

            // Adding the course using ModuleImpl
            ModuleImpl module = new ModuleImpl();
            try {
                Cources newCourse = module.addCourse(course);
                out.println("Course added successfully with ID: " + newCourse.getId());
            } catch (Exception ex) {
                out.println("Failed to add course: " + ex.getMessage());
            }
        }
    }
    ...
    public String getServletInfo() {
        return "Servlet for adding a new course";
    }
}
